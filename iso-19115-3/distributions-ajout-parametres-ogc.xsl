...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="cit:CI_OnlineResource[some $p in $protocols satisfies
                         starts-with(lower-case(cit:protocol/gco:CharacterString), concat('ogc:', $p))
                       ]/cit:linkage/gco:CharacterString">

    <xsl:variable name="protocol" select="lower-case(ancestor::cit:CI_OnlineResource/cit:protocol/gco:CharacterString)"/>
